...
def index(request):
    ...
    items = Appliance.objects.filter(category_id__in=categories)
...
def items_with_category(request, id):
    items = Appliance.objects.filter(category_id=id)
    items_all = Appliance.objects.filter(category_id=id)
...
def items(request):
    items_all = Appliance.objects.all()
    categories = Category.objects.all()
    subcategories = Subcategory.objects.all()
    persons = ResponsiblePerson.objects.all()
...
def delete_certificate(request):
    if request.method == 'POST':
        ...
        certificate = Certificate.objects.filter(id=certificate_id).first()
        certificate.delete()
        history = History(date=datetime.now(),appliance_id=appliance_id,user_id=user_delete_id,status=4,comment="Serficat o'chirildi")
        history.save()
        messages.success(request, "Sertificat o'chirildi !")
        return HttpResponse(1)
...
def search(request):
    if request.method == 'POST':
        ...
        if status_value:
            ...
            certificates = Certificate.objects.filter(checked_organization__contains=belongs_to_item)
            items = 0
            ...
            certificates = Certificate.objects.filter(expiry_date__range=(date_b, date_e))
            items = 0
    ...
    if request.method == 'POST':
        ...
        if status_value:
            if status_value == '1':
                ...
                certificates = Certificate.objects.filter(expiry_date__year=current_year,expiry_date__month=current_month,
            ...
            certificates = Certificate.objects.filter(checked_organization__contains=checked_organization).filter(appliance__category_id=category_id)
