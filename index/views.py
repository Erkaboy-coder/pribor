...
def items_with_category(request, id):
    ...
    context = {'items':items, 'category':category,'items_all':items_all,'subcategories':subcategories,'persons': persons,'current_year':current_year,'current_month':current_month,'current_day':current_day}
...
def items(request):
    ...
    context = {'items_all':items_all, 'categories':categories, 'subcategories':subcategories, 'persons':persons,'current_year':current_year,'current_month':current_month,'current_day':current_day}
...
def edit_item(request):
    if request.method == 'POST':
        ...
        appliance.category_id=category
        appliance.subcategory_id=subcategory
        appliance.name=name1
        appliance.inv_number=inv_number
        appliance.factory_number=factory_number
        appliance.origin_country=origin_country
        appliance.origin_factory=origin_factory
        appliance.origin_year=origin_year
        appliance.used_year=used_year
        appliance.status_battery=status_battery
        appliance.battery=status_battery_value
        appliance.status_passport=status_passport_value
        appliance.passport=passport
        appliance.department_id=exploit_department
        appliance.responsible_user=responsible_user
        appliance.info=info
        appliance.compatibility_file=compatibility_file
        appliance.status_compatibility_file_value=status_compatibility_file_value

        appliance.save()

        history = History(date=datetime.now(), status=9, user_id=user_id, appliance_id=id)
        # status = 9 bolsa pribor ma'lumotlari o'zgartirilgan
        history.save()

        messages.success(request, "Pribor ma'lumotlari o'zgartirildi! ")
        return HttpResponseRedirect(href)
    else:
...
def show(request,id):
    ...
    certificates = Certificate.objects.filter(appliance=id).order_by('-id')
    current_year = datetime.datetime.now().year
    current_day = datetime.datetime.now().day
    current_month = datetime.datetime.now().month
...
def item_edit(request, id):
    ...
    item = Appliance.objects.filter(id=id).first()
    category = Category.objects.filter(id=item.category.id).first()
    certificates = Certificate.objects.filter(appliance_id=id)
    subcategories = Subcategory.objects.all()
    departments = Departments.objects.all()
    responsible_persons = ResponsiblePerson.objects.all()
    ...
    context = {'category': category, 'departments': departments, 'subcategories': subcategories,
               'persons': responsible_persons,'item': item, 'certificates':certificates,'current_year':current_year,'current_month':current_month,'current_day':current_day}
...
# def delete_by_certificate(request):
#     if request.method == 'POST':
#         data = request.POST
#         id = data.get('data-id')
#         user_id = data.get('data-user')
...
#             i.delete()
#
#         user_type=Profile.objects.filter(user_id=user_id).first()
#         history = History(date=datetime.now(), status=8, appliance_id=id, user_id=user_id, comment="Priborni o'chirgan")
#         history.save()
#         messages.success(request, "Pribor o'chirildi !")
#         return HttpResponse(1)
...
def edit_certificate(request):
    if request.method == 'POST':
        ...
        print(issue_date_edit)
        print(expiry_date_edit)
    ...
    if request.method == 'POST':
        ...
        if status_value:
            if status_value == '1':
                ...
                items = Appliance.objects.all().filter(is_delete=0)
            elif status_value == '2':
                # status_value == '2': yaroqli priborlar
                items = Appliance.objects.filter(Q(status=0) | Q(status=7)).filter(is_delete=0)
            elif status_value == '5':
                items = Appliance.objects.filter(status=2).filter(is_delete=0)
                # status_value == '2': yaroqsiz priborlar
            elif status_value == '6':
                items = Appliance.objects.filter(status=1).filter(is_delete=0)
                # status_value == '6': vaqtincha yaroqsizlar

            elif status_value == '7':
                items = Appliance.objects.filter(status=3).filter(is_delete=0)
                ...
                certificates = Certificate.objects.filter(expiry_date__year=current_year,
                                                          ...
                                                          expiry_date__day__gt=current_day).filter(appliance__is_delete=0).filter(last=1)
                items = 0
            elif status_value == '4':
                certificates = Certificate.objects.filter(
                    expiry_date__year__lt=current_year).filter(appliance__is_delete=0).filter(last=1) or Certificate.objects.filter(
                    expiry_date__year=current_year,
                    expiry_date__month__lt=current_month).filter(appliance__is_delete=0).filter(last=1) or Certificate.objects.filter(
                    expiry_date__year=current_year,
                    expiry_date__month=current_month,
                    expiry_date__day__lte=current_day).filter(appliance__is_delete=0).filter(last=1)
            ...
            if status_passport_value == '1':
                items = Appliance.objects.filter(status_passport=1).filter(is_delete=0)
            elif status_passport_value == '2':
                items = Appliance.objects.filter(status_passport=0).filter(is_delete=0)
            else:
                items = 0

        elif name_item:
            items = Appliance.objects.filter(name__contains=name_item).filter(is_delete=0)

        elif belongs_to_item:
            certificates = Certificate.objects.filter(checked_organization__contains=belongs_to_item).filter(appliance__is_delete=0).filter(last=1)
            items = 0

        elif responsible_user:
            items = Appliance.objects.filter(responsible_user=responsible_user).filter(is_delete=0)
        elif id_number_item:
            items = Appliance.objects.filter(inv_number__contains=id_number_item).filter(is_delete=0)
            ...
            certificates = Certificate.objects.filter(expiry_date__range=(date_b, date_e)).filter(appliance__is_delete=0).filter(last=1)
            items = 0

        elif category_id != '-1':
            items = Appliance.objects.filter(category_id=category_id).filter(is_delete=0)

        elif department_search != 0:
            items = Appliance.objects.filter(subcategory=department_search).filter(is_delete=0)
        else:
            items = 0
        context = {'items': items, 'category': category, 'certificates': certificates, 'subcategories': subcategories,
                   'persons': persons,'categories':categories,'current_year':current_year,'current_month':current_month,'current_day':current_day}
        return render(request, 'items.html', context)
    else:
        return redirect('/')
    ...
    if request.method == 'POST':
        ...
        if status_value:
            if status_value == '1':
                ...
                items = Appliance.objects.filter(Q(status=0) | Q(status=7)).filter(category_id=category_id).filter(is_delete=0)
            elif status_value == '5':
                items = Appliance.objects.filter(status=2).filter(category_id=category_id).filter(is_delete=0)

            elif status_value == '6':
                items = Appliance.objects.filter(status=1).filter(category_id=category_id).filter(is_delete=0)
                ...
                certificates = Certificate.objects.filter(expiry_date__year=current_year,
                                                          expiry_date__month=current_month,
                                                          expiry_date__day__gt=current_day).filter(appliance__category_id=category_id).filter(appliance__is_delete=0).filter(last=1)
                ...
                certificates = Certificate.objects.filter(
                    expiry_date__year__lt=current_year).filter(appliance__category_id=category_id).filter(appliance__is_delete=0).filter(last=1) or Certificate.objects.filter(
                    expiry_date__year=current_year,
                    expiry_date__month__lt=current_month).filter(appliance__is_delete=0).filter(appliance__category_id=category_id).filter(last=1) or Certificate.objects.filter(
                    expiry_date__year=current_year,
                    expiry_date__month=current_month,
                    expiry_date__day__lte=current_day).filter(appliance__category_id=category_id).filter(appliance__is_delete=0).filter(last=1)
            ...
            if status_passport_value == '1':
                items = Appliance.objects.filter(status_passport=1).filter(category_id=category_id).filter(is_delete=0)
            elif status_passport_value == '2':
                items = Appliance.objects.filter(status_passport=0).filter(category_id=category_id).filter(is_delete=0)

        elif name_item:
            items = Appliance.objects.filter(name__contains=name_item).filter(category_id=category_id).filter(is_delete=0)

        elif checked_organization:
            certificates = Certificate.objects.filter(checked_organization__contains=checked_organization).filter(appliance__is_delete=0).filter(appliance__category_id=category_id).filter(last=1)
            ...
            items = Appliance.objects.filter(responsible_user=responsible_user).filter(category_id=category_id).filter(is_delete=0)

        elif id_number_item:
            items = Appliance.objects.filter(inv_number__contains=id_number_item).filter(category_id=category_id).filter(is_delete=0)

        elif issue_date_item and expiry_date_item:
            date_b = datetime.datetime.strptime(request.POST.get('issue_date_item'), '%Y-%m-%d')
            date_e = datetime.datetime.strptime(request.POST.get('expiry_date_item'), '%Y-%m-%d')
            certificates =Certificate.objects.filter(expiry_date__range=(date_b, date_e)).filter(appliance__is_delete=0).filter(last=1).filter(appliance__category_id=category_id)
            items = 0
            # items = Appliance.objects.filter(category_id=category_id).filter(appliances__expiry_date__range=(date_b, date_e))

        elif department_search != 0:
            items = Appliance.objects.filter(subcategory=department_search).filter(category_id=category_id).filter(is_delete=0)
        else:
            items = 0
        context = {'items': items, 'category': category,'certificates':certificates,'subcategories':subcategories,'persons':persons,'current_year':current_year,'current_month':current_month,'current_day':current_day}
        return render(request,'items_with_category.html',context)
    else:
        return redirect('index')
...
def send(request):
    if request.method == 'POST':
        ...
        # user_name_key = force_text(urlsafe_base64_decode(stir))
        user_name_code = urlsafe_base64_encode(force_bytes(user_name))
        user = Profile.objects.filter(user__username=user_name).filter(user__email=user_email).first()
        if user:
            try:
                context = {'user_name': user_name,'user_email': user_email,'user_name_code': user_name_code}
                template = render_to_string('email_message.html', context)
                text_content = strip_tags(template)
                email = EmailMultiAlternatives(
                    ' Parolni tiklash havolasi!',
                    text_content,
                    'mail.dshk.uz@gmail.com',
                    [user_email]
                )
                email.attach_alternative(template, 'text/html')
                email.send()

                messages.success(request, "Sizga parolni tiklash havolasi yuborildi")
                return HttpResponse(1)
            except gaierror as e:
                messages.error(request, "Sizda internet bilan muommo bor! ")
                return HttpResponse(1)
        else:
            messages.error(request, "Bunday foydalanuvchi mavjud emas! ")
            return HttpResponse(0)
...
def reset_password(request,user):
    user_name = force_text(urlsafe_base64_decode(user))
    user = Profile.objects.filter(user__username=user_name).first()
    if user:
        context = {'user_name': user_name,'user':user}
...
def password_change(request):
    if request.method == 'POST':
        ...
        user.save()
        messages.success(request, "Tabriklaymiz parol muvoffaqiyatli o'zgartirildi! ")
        return HttpResponseRedirect('/')
    else:
        return HttpResponseRedirect('/')
